...
int D3d_view (double view[4][4],  double view_inverse[4][4],
  ...
  double temp[4][4], temp_inverse[4][4];

  D3d_make_identity(temp);
  D3d_make_identity(temp_inverse);
  D3d_make_identity(view);
  D3d_make_identity(view_inverse);

  //translate so eye is at origin 
  D3d_translate(temp, temp_inverse, -eye[0], -eye[1], -eye[2]);


  D3d_mat_mult(view,  temp,view);
  D3d_mat_mult(view_inverse, view_inverse, temp_inverse);
  D3d_mat_mult_pt(tup, temp, up);
  D3d_mat_mult_pt(tcoi, temp, coi);

  D3d_make_identity(temp);
  D3d_make_identity(temp_inverse);

  // //rotate around y axis so that coi' is in the y-z plane 
  double p = sqrt((tcoi[0]*tcoi[0]) + (tcoi[2] * tcoi[2]));
  D3d_cs_rotate_y(temp, temp_inverse, tcoi[2]/p, -tcoi[0]/p );
 

   D3d_mat_mult(view,  temp,view);
  D3d_mat_mult(view_inverse, view_inverse,temp_inverse);
  D3d_mat_mult_pt(tup, temp, tup);
  D3d_mat_mult_pt(tcoi, temp, tcoi);
