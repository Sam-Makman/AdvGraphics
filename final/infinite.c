...
void fractal(SHAPE seed, int childNum, double m[SHOWN][4][4], double minv[SHOWN][4][4]){
  int i;
  // printf("%d \n", childNum);
  if(childNum >= (SHOWN - 1 - seed.numChildren)){
    // printf("return\n");
    return;
  }
  printf("%d\n",childNum );
      make_matrix(seed,m[childNum],minv[childNum]);
  // double m[1024][4][4], minv[1024][4][4];
  for(i=childNum + 1; i < seed.numChildren + childNum; i++){
    SHAPE child = get_child(seed, i%seed.numChildren);

    fractal(seed,childNum + (i*child.numChildren),m,minv);
  }
...
int main(){
  ...
  G_clear();
   ...
   for(i=0;i<1;i++){
    ...
    color[1] = 1;
    color[2] = 1;
    SHAPE seed = new_shape(0,0,150,30,color, .5,5);

    fractal(seed,0,m,minv);
    printf("end fractal\n");
